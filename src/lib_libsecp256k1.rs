...
use libsecp256k1::curve::{Affine, Scalar, AFFINE_G};
// use libsecp256k1::

struct LocalSignature {
    pub r: Scalar,   //FE,
    pub R: Affine,   //GE,   //pointF
    pub s_i: Scalar, //FE,  //scalara
    pub m: BigInt,
    pub y: Affine, //GE,
}
...
pub fn combine_signature(R_x: &str, R_y: &str, shares: &str) -> String {
    ...
    let mut shares_raw: Vec<[u8; 32]> = serde_json::from_str(&shares).unwrap();
    let mut shares: Vec<Scalar>;

    for s in shares_raw {
        let s2 = Scalar::from_int(0);
        s2.set_b32(&s.clone());
        shares.push(s2);
    }
...
pub fn combine_signature_internal(
    ...
    let s_bn: BigInt = BigInt::from_bytes_be(&s.b32());
    //    let s_bn = s.to_big_int();

    let group_order = CURVE_ORDER;

    let l_i = &local_x
        .mod_floor(&BigInt::from_bytes_be(&group_order))
        .to_bytes_be().into_iter().collect()

    let r: Scalar = Scalar::from_int(0);
    r.set_b32(l_i);
